...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            place.ant = self
        elif place.ant.can_contain(self) and self.is_container ==False:
            place.ant.store_ant(self)
        elif place.ant.is_container == False and self.can_contain(place.ant):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while curr is not None:
            ...
            if curr.is_hive:
                return None
            elif curr.bees and self.lower_bound  <= distance <=  self.upper_bound:
                return random_bee(curr.bees)
            else:
                curr = curr.entrance
                distance += 1
        return None
